...
{
    HRESULT GetProjectionMatrix(__out DirectX::XMMATRIX *projection);
    HRESULT SetProjectionMatrix(DirectX::CXMMATRIX projection);
	HRESULT GetViewMatrix(DirectX::XMMATRIX *viewMatrix);
	...
	HRESULT SetPositionAndScale(DirectX::CXMVECTOR pos, FLOAT scale);
	HRESULT SetPositionAtUpFov(DirectX::CXMVECTOR pos, DirectX::CXMVECTOR at, DirectX::CXMVECTOR up, FLOAT fov);
};
...
HRESULT CreateSceneGraphEditor(__in ISceneGraph *pSceneGraph, __deref_out ISceneGraphEditor **ppSceneGraphEditor);
