...
class VertexStream : public ImplementSmartObject
    <
        VertexStream, 
        ClassFlags<CF_ALIGNED_MEMORY>,
        InheritImplementation<MeshStream>, 
        IVertexStream
    >
{
public:

	HRESULT Initialize(__in const UINT length, __deref_in_ecount(length) const VertexType* pInitialData);
	HRESULT Lock(VertexType **ppBuffer);
	HRESULT Unlock();
};