...
const D3D11_INPUT_ELEMENT_DESC FontShader::m_InputLayout[] = 
{
...
HRESULT FontShader::InitializeConstantBuffers(__in ID3D11Device* pDevice)
{
	HRESULT hr;
...
HRESULT FontShader::Initialize(__in ID3D11Device* pDevice, __in ID3D11DeviceContext* pDeviceContext)
{
	return InitializeInternal(pDevice, pDeviceContext, g_vsfontshader, sizeof(g_vsfontshader), g_psfontshader, sizeof(g_psfontshader), m_InputLayout, sizeof(m_InputLayout));
}

HRESULT FontShader::SetShaderParametersAndTexture(__in CXMMATRIX worldMatrix, __in CXMMATRIX viewMatrix, __in CXMMATRIX projectionMatrix, __in ID3D11ShaderResourceView* texture)
{
...
{
    return FontShader::CreateInstance(ppResult, pDevice, pDeviceContext);
}